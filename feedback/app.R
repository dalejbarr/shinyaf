...
### create a new course
      if (mode == modekey_create) {
        ...
        fluidPage(
          conditionalPanel("input.submit == 0",
                           ...
                           textInput("simplepush_key", "Simplepush app key for mobile notifications (optional; leave blank if you don't have one):"),
                           p("[note: simplepush notifications will have the event id 'feedback']"),
  ...
  observeEvent(input$post, {    
    ...
    if ((sp_key <- get_field("simplepush_key", tokens$student)) != "") {
      httr::POST("https://api.simplepush.io/send",
                 encode = "form",
                 body = list(key = sp_key,
                             title= "Student Feedback Received",
                             msg= sprintf("You received feedback for %s.",
                                          tokens$coursename),
                             event = "feedback"))
    }
  })
